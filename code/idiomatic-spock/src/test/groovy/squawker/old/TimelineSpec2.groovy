...
class TimelineSpec2 extends Specification {
  ...
  def "when new messages are posted they appear in the timeline"() {
    ...
    def oldSize = timeline.size()
    def oldTimestamp = timeline.first().postedAt

    when:
    postMessageBy(followedUser)

    then:
    timeline.size() == oldSize + 1
    timeline.first().postedAt > oldTimestamp
  }
  // end::new-old-variables[]
}
