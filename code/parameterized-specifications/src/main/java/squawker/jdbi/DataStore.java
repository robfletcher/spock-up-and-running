...
public abstract class DataStore implements GetHandle {
  ...
  @SqlUpdate("create table message (id char(36) primary key,"
    + "                      posted_by_id int,"
    + "                      text varchar(140),"
    + "                      posted_at timestamp)")
  ...
  public User findUser(@Bind("username") String username) {
    return getHandle()
      .createQuery("select username, registered from user where username = :username")
  ...
  public User newUser(String username) {
    User user = new PersistentUser(this, username);
    insert(user);
    return user;
  }
  ...
  @SqlUpdate("insert into message (id, posted_by_id, text, posted_at)"
    + " select :id, u.id, :text, :postedAt from user u"
  ...
  @SqlQuery("select 1 from user where username = :username")
  public abstract boolean usernameInUse(@Bind String username);
}
