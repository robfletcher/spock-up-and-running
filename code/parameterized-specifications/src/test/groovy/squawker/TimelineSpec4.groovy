...
class TimelineSpec4 extends BasePersistenceSpec {
  ...
  // tag::unroll-description[]
  @Unroll("a message posted by #postedBy #behavior in #whose timeline") // <3>
  def "a user only sees messages from users they follow in their timeline"() {
    ...
    behavior = shouldAppearInTimeline ? "appears" : "does not appear" // <1>
    whose = postedBy == user ? "their own" : "$user's" // <2>
  }
  // end::unroll-description[]
}
