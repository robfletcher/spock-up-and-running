...
public class TimelineStream {
  ...
  // tag::last-message-id[]
  private Serializable lastMessageId = null; // <1>

  public void start() {
    Observable
      .interval(interval, intervalUnit, scheduler)
      .flatMapIterable(tick -> squawker.getTimeline(username, lastMessageId)) // <2>
      .doOnNext(message -> lastMessageId = message.getId()) // <3>
